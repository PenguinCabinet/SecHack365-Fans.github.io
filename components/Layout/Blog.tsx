...
const Blog = ({
  ...
  const title: string = `SecHack365 ${year}年度 参加記 まとめ`;
  const description: string = `${year}年度にSecHack365に参加したトレーニーによる参加ブログのまとめです。`;
  return (
    <Layout title={title} description={description}>
      <h1>{year}年度 SecHack365 参加記 まとめ【随時更新】</h1>
      <p>
        {year}
        年度にSecHack365に参加したトレーニーによる参加ブログのまとめです。(順不同)
      </p>
      {Courses.map((course) => (
